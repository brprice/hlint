...
simplifyExp e@(L _ (HsLet _ _ ((HsValBinds _ (ValBinds _ binds []))) _ z)) =
  ...
  case bagToList binds of
    [L _ (FunBind _ _ (MG _ (L _ [L _ (Match _(FunRhs (L _ x) _ _) [] (GRHSs _[L _ (GRHS _ [] y)] ((EmptyLocalBinds _))))])) _)]
...
replaceBranches (L s (HsCase _ a (MG FromSource (L l bs)))) =
  (concatMap f bs, \xs -> L s (HsCase EpAnnNotUsed a (MG Generated (L l (g bs xs)))))
  where
